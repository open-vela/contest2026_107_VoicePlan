...
应用使用官方 `system.record` 录制最多 5 秒、16 kHz、单声道 WAV，经 `system.file` 读取后，由手表 `system.fetch` 通过 HTTPS 调用 MiMo 多模态转写。录音使用 256000 bps 的 WAV 配置，并允许最多 1 MiB 的设备音频文件。返回文字先进入输入框，用户确认或编辑后再生成计划。未配置、断网、超时或无清晰语音时保留原目标，不用示例冒充识别结果。
